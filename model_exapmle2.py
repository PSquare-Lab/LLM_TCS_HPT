# #!/usr/bin/env python3
# """
# ResNet-18 Butterfly Classification Model with Optimized Hyperparameters - FIXED VERSION

# This script implements a ResNet-18 model for butterfly image classification.
# It uses optimized hyperparameters from the phased optimization process.
# """
...
#     """Load hyperparameters from the optimization pipeline with optimized values support."""
...
#     # Use optimized values when available
...
#         # Filter out images with dummy labels (-1) for validation
#         self.valid_indices = []
...
#         # Clean up path - remove any duplicate directories
...
#         # CRITICAL FIX: Filter out missing files BEFORE label encoding
#         print("🔍 Filtering missing files...")
        
#         # Filter training data - only keep files that actually exist
#         train_exists_mask = train_df['full_path'].apply(os.path.exists)
#         train_df_filtered = train_df[train_exists_mask].copy()
        
#         # Filter test data - only keep files that actually exist
...
#         # Now encode labels using only the filtered data
#         le = LabelEncoder()
#         train_df_filtered['label_encoded'] = le.fit_transform(train_df_filtered['label'])
        
#         # Only encode test labels if they exist
#         if has_test_labels and len(test_df_filtered) > 0:
#             # Only transform labels that exist in training set
...
#         # Verify some sample paths exist (should all be True now)
...
# def load_butterfly_data(data_dir, batch_size, test_size=0.2, val_size=0.1):
#     """Load and prepare butterfly dataset. FIXED VERSION."""
#     try:
...
#     """ResNet-18 with configurable dropout for regularization."""
    
#     def __init__(self, num_classes=10, dropout_rate=0.0, pretrained=True):
...
# def train_model():
#     """Main training function - FIXED VERSION with reduced output."""
#     hyperparams = get_hyperparameters()
    
#     # Print optimization info (reduced)
...
#     print(f"📊 LR: {learning_rate}, BS: {batch_size}, WD: {weight_decay}, DR: {dropout_rate}")
...
#     print(f"🚀 Starting training for {epochs} epochs with {optimizer_type}")
...
#     # Training loop with reduced output
...
#         # Print only every 10 epochs or last epoch
#         if (epoch + 1) % 10 == 0 or epoch == epochs - 1:
#             print(f"Epoch {epoch+1}/{epochs} - Train: {train_accuracy:.3f}, Val: {val_accuracy:.3f}")
...
#     if optimized_used:
#         print(f"✅ Optimized: {', '.join(optimized_used)}")
...
#!/usr/bin/env python3
"""
ResNet-18 Butterfly Classification Model - STRICTLY FOLLOWING ORIGINAL PAPER

This script implements ResNet-18 following the exact specifications from:
Kaiming He, Xiangyu Zhang, Shaoqing Ren, and Jian Sun. 
"Deep residual learning for image recognition." 
In Proceedings of the IEEE conference on computer vision and pattern recognition, pages 770–778, 2016.

Key changes to follow paper:
- Only paper-specified data augmentation (random crop, horizontal flip)
- SGD optimizer with momentum (as specified in paper)
- Specific learning rate schedule from paper
- Global pooling as tunable hyperparameter
- Removed non-paper augmentations (ColorJitter, rotation)
- Kaiming initialization (paper standard)
"""
...
#         'global_pool': 'avg'         # New tunable parameter: 'avg' or 'max'
#     }
...
#     - Training: Random crop (224x224), random horizontal flip, normalization
#     - Testing: Center crop (224x224), normalization
#     - NO ColorJitter, NO rotation, NO other augmentations not in paper
#     """
#     try:
#         print("🦋 Loading butterfly dataset with paper-compliant augmentation...")
...
#         # From original paper: "The image is resized with its shorter side randomly sampled in [256, 480] for scale augmentation"
#         # "A 224×224 crop is randomly sampled from an image or its horizontal flip"
#         transform_train = transforms.Compose([
#             transforms.Resize(256),                    # Resize shorter side to 256
#             transforms.RandomCrop(224),                # Random 224x224 crop (paper specified)
#             transforms.RandomHorizontalFlip(p=0.5),    # Random horizontal flip (paper specified)
#             transforms.ToTensor(),
#             transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # ImageNet normalization
#         ])
        
#         # For testing: center crop as specified in paper
#         transform_test = transforms.Compose([
#             transforms.Resize(256),                    # Resize shorter side
#             transforms.CenterCrop(224),                # Center crop 224x224 (paper specified)
#             transforms.ToTensor(),
#             transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
#         ])
        
#         print("📊 Using paper-compliant augmentation:")
#         print("   - Training: Resize(256) → RandomCrop(224) → RandomHorizontalFlip")
#         print("   - Testing: Resize(256) → CenterCrop(224)")
...
#     Key changes:
#     - Configurable global pooling (avg or max) as tunable hyperparameter
#     - Optional dropout for regularization (not in original paper, but commonly added)
#     - Kaiming initialization (paper standard)
#     - Maintains all other paper specifications
#     """
...
#         # Apply Kaiming initialization (paper standard)
#         self._initialize_weights()
    
#     def _initialize_weights(self):
#         """Apply Kaiming initialization as specified in the paper."""
...
#     def forward(self, x):
#         # Forward through ResNet backbone (without final layers)
#         x = self.resnet(x)
...
#     Following the original ResNet paper specifications:
#     - SGD optimizer with momentum 0.9
#     - Weight decay 1e-4 
#     - Learning rate starts at 0.1, divided by 10 when error plateaus
#     - Batch size (tunable, paper used 256 but can be adjusted for hardware)
#     """
#     hyperparams = get_hyperparameters()
...
#     # Extract hyperparameters with paper-compliant defaults
#     learning_rate = hyperparams.get('learning_rate', 0.1)      # Paper default: 0.1
#     batch_size = hyperparams.get('batch_size', 32)             # Adjusted for smaller datasets
#     epochs = hyperparams.get('epochs', 50)
#     weight_decay = hyperparams.get('weight_decay', 0.0001)     # Paper default: 1e-4
#     momentum = hyperparams.get('momentum', 0.9)                # Paper default: 0.9
#     optimizer_type = hyperparams.get('optimizer', 'SGD')       # Paper uses SGD
#     dropout_rate = hyperparams.get('dropout_rate', 0.0)        # Not in paper, but useful
#     global_pool = hyperparams.get('global_pool', 'avg')        # New tunable parameter
#     data_dir = hyperparams.get('data_dir', './butterfly_data')
...
#     # Warn if non-paper settings are used
#     if optimizer_type.lower() != 'sgd':
...
#     # Initialize model with paper-compliant architecture
#     model = ResNetPaperCompliant(
#         num_classes=num_classes, 
#         dropout_rate=dropout_rate,
#         global_pool=global_pool,
#         pretrained=True  # Start with pretrained weights, then fine-tune
#     ).to(device)
    
#     # Loss function (paper standard)
#     criterion = nn.CrossEntropyLoss()
    
#     # Optimizer - PAPER COMPLIANT (SGD with momentum)
#     if optimizer_type.lower() == 'sgd':
#         optimizer = optim.SGD(
#             model.parameters(), 
#             lr=learning_rate, 
...
#     else:
#         # Allow other optimizers but warn
#         print(f"⚠️ Using {optimizer_type} (not paper-compliant)")
#         optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    
#     # Learning rate scheduler - Paper uses manual scheduling
#     # "We divide the learning rate by 10 when the error plateaus"
#     scheduler = optim.lr_scheduler.ReduceLROnPlateau(
#         optimizer, mode='min', factor=0.1, patience=10
#     )
    
#     print(f"🚀 Starting paper-compliant training for {epochs} epochs")
#     print(f"📚 Following: He et al. 'Deep residual learning for image recognition' (2016)")
...
#         # Step scheduler with validation loss (paper-style)
...
#         print("📚 Based on: He et al. 'Deep residual learning for image recognition' (2016)")
#         print("🔧 Key changes:")
#         print("   - Strict paper-compliant data augmentation")
#         print("   - SGD optimizer with paper settings")
#         print("   - Global pooling as tunable hyperparameter")
#         print("   - Kaiming initialization")
#         print("   - Removed non-paper augmentations")
#         print()
